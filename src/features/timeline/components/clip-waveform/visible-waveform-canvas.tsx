...
function scheduleLiveCanvasPositionUpdate(): void {
  ...
  queueMicrotask(() => {
    ...
    for (const registration of liveCanvasRegistrations) {
      ...
      const geometry = measureLiveTimelineGeometry(registration.canvas, registration.overscanPx)
      if (geometry && geometry.width > 0) {
        ...
        updates.push({
          ...
          geometry,
          needsRedraw:
            registration.canvas.style.display === 'none' || currentWidth + 0.5 < geometry.width,
        })
      }
    }
...
export const VisibleWaveformCanvas = memo(function VisibleWaveformCanvas({
  ...
  useLayoutEffect(() => {
    ...
    if (liveTimelineViewport) {
      geometry = measureLiveTimelineGeometry(canvas, liveViewportOverscanPx) ?? geometry
    }
    redrawGeometryRef.current(geometry)
  }, [height, liveTimelineViewport, liveViewportOverscanPx, renderWindow, version, viewportVersion])
