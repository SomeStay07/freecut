...
/**
 * Baseline probe spanning the scripts our projects commonly render. Google Fonts serves
 * each family as per-script SUBSET faces (latin / cyrillic / ...): loading with a
 * latin-only probe ("BESbswy") activates only the latin face, and the cyrillic face then
 * lazy-loads MID-RENDER on the first Cyrillic glyph — a visible glyph swap seconds into
 * the video. We always load/check with the REAL characters the composition draws
 * (collectVisibleTextSample) so any script is covered; this baseline is unioned in as a
 * safety net for dynamic text.
 */
const FONT_SETTLE_BASELINE = 'BESbswy ЁёЙйАбвгд 0123456789 $%+−?!'

/**
 * Every unique character the composition's text items will draw — the honest invariant
 * for font readiness ("settled for every glyph we render"), independent of script.
 */
function collectVisibleTextSample(tracks: TimelineTrack[]): string {
  const chars = new Set<string>(FONT_SETTLE_BASELINE)
  for (const track of tracks) {
    for (const item of track.items) {
      if (item.type !== 'text' && item.type !== 'subtitle') continue
      const textItem = item as { text?: string; textSpans?: Array<{ text?: string }> }
      for (const ch of textItem.text ?? '') chars.add(ch)
      for (const span of textItem.textSpans ?? []) for (const ch of span.text ?? '') chars.add(ch)
    }
  }
  return [...chars].join('')
}
...
/**
 * Wait until every requested family/weight is ACTUALLY activated for every script we
 * draw, not merely requested. Two traps this handles:
 * 1) `document.fonts.load()` resolves empty before the injected Google-Fonts stylesheet
 *    is parsed (no @font-face known yet) — detected by inspecting FontFace entries.
 * 2) Subset lazy-loading (see FONT_SETTLE_SAMPLE) — handled by loading/checking with a
 *    Cyrillic-bearing sample so `document.fonts.check(spec, sample)` only passes when
 *    ALL faces needed for that text are loaded.
 */
async function awaitFontsSettled(
  fontNames: readonly string[],
  weights: readonly number[],
  sample: string,
): Promise<void> {
  if (fontNames.length === 0 || typeof document === 'undefined' || !document.fonts) return
  const deadline = Date.now() + 15_000
  for (;;) {
    await ensureFontsLoaded(fontNames, weights)
    // Force-load every subset the sample needs (latin probe alone leaves cyrillic lazy).
    await Promise.all(
      fontNames.flatMap((name) =>
        weights.map((weight) =>
          document.fonts.load(`${weight} 16px "${name.trim()}"`, sample).catch(() => []),
        ),
      ),
    )
    ...
    const settled = fontNames.every(
      (name) =>
        loadedFamilies.has(name.trim().toLowerCase()) &&
        weights.every((weight) => document.fonts.check(`${weight} 16px "${name.trim()}"`, sample)),
    )
    if (settled) return
    if (Date.now() > deadline) {
      // Never fail the render over fonts, but never be silent either: a fallback-font
      // frame is a real fidelity defect the operator must see in the logs.
      const unsettled = fontNames.filter(
        (name) =>
          !weights.every((weight) =>
            document.fonts.check(`${weight} 16px "${name.trim()}"`, sample),
          ),
      )
      log.warn('Fonts not settled before render; output may use fallback glyphs', {
        unsettled,
      })
      return
    }
...
async function renderTimeline(input: HeadlessTimelineInput): Promise<HeadlessRenderSummary> {
  ...
  await awaitFontsSettled(
    [...new Set([DEFAULT_TEXT_FONT_FAMILY, ...collectVisibleTextFontFamilies(composition.tracks)])],
    HEADLESS_FONT_WEIGHTS,
    collectVisibleTextSample(composition.tracks),
  )
