...
const IMAGE_MIME_BY_FORMAT = { png: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg', webp: 'image/webp' }
...
async function main() {
  ...
  const handleFrame = async (req, res) => {
    const body = await readJsonBody(req)
    const project = body.projectObject ?? loadJobProject(workspace, { project: body.project }).project
    ...
    const outPath = path.join(tmpDir, `frame-${process.pid}-${++counter}.${IMAGE_EXT_BY_MIME[mime]}`)
